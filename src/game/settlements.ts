...
function route(config: WorldConfig, shell: SettlementShell, start: WorldCoordinate, target: WorldCoordinate, existing: Set<string>, fieldCache: Map<string, GeographicFields>) {
  ...
  while (frontier.length) {
    ...
    for (const [dx, dy] of DIRECTIONS) {
      ...
      const fields = fieldsAtPoint(x, y); const walkable = fields.elevation >= 0.28 && fields.slope <= 0.14; if (!walkable) continue;
...
export function generateSettlementLayout(config: WorldConfig, shell: SettlementShell): SettlementLayout {
  ...
  for (const [index, item] of required.entries()) buildings.push({ id: `${shell.id}:building:anchor:${index}`, type: item.type, districtId: item.district.id, x: item.x, y: item.y, width: 3, height: 3, rotation: 0, roadId: null, courtyard: false });
