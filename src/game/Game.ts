...
import { advanceAdventurer, createAdventurer, sampleAdventurer, type Adventurer, type AdventurerLod, type TravelTopology } from './adventurers';
...
export class Game {
  ...
  private buildHero() {
    ...
    this.hero.addChild(figure); this.boat.addChild(wake, hull); this.boat.visible = false; this.actor.addChild(shadow, this.hero, this.boat);
  }
  private navigation() { const roadTileKeys = new Set<string>(); const bridgeTileKeys = new Set<string>(); const waterRouteTileKeys = new Set<string>(); const portTileKeys = new Set<string>(); const portLinks = new Map<string, Set<string>>(); const chunks = [...this.displays.values()].map((display) => display.chunk).concat([...this.prefetched.values()]); for (const chunk of chunks) { for (const tile of chunk.tiles) { if (tile.road) roadTileKeys.add(key(tile.x, tile.y)); if (tile.port) portTileKeys.add(key(tile.x, tile.y)); if (tile.waterRoute) waterRouteTileKeys.add(key(tile.x, tile.y)); } for (const road of chunk.roads) { for (const bridge of road.bridges) for (const tile of bridge.tiles) bridgeTileKeys.add(key(tile.x, tile.y)); for (const port of road.ports) { const portKey = key(port.x, port.y); const links = portLinks.get(portKey) ?? new Set<string>(); for (const water of port.waterTiles) links.add(key(water.x, water.y)); portLinks.set(portKey, links); } } } return { roadTileKeys, bridgeTileKeys, waterRouteTileKeys, portTileKeys, portLinks }; }
  ...
  private adventurerLod(adventurer: Adventurer, bounds: ReturnType<Game['visibleBounds']>): AdventurerLod {
    const settlement = this.adventurerTopology?.settlements.find((candidate) => candidate.id === adventurer.currentSettlementId);
    const sampled = sampleAdventurer(adventurer, this.gameTime); const point = sampled ?? settlement ?? { x: this.px, y: this.py };
  ...
  private simulateAdventurers(stepSeconds: number) {
    ...
    for (const adventurer of this.adventurers.values()) { adventurer.lod = this.adventurerLod(adventurer, bounds); const phase = adventurer.tickPhase; const due = adventurer.lod === 'live' || (adventurer.lod === 'coarse' && this.simulationStep % 5 === phase % 5) || (adventurer.lod === 'sparse' && this.simulationStep % 30 === phase % 30); if (due) advanceAdventurer(adventurer, this.adventurerTopology, this.gameTime); }
    for (const goblin of this.goblins.values()) { const point = { x: goblin.x, y: goblin.y }; const cx = Math.floor(point.x / CHUNK_SIZE); const cy = Math.floor(point.y / CHUNK_SIZE); const ring = Math.max(cx < bounds.minChunkX ? bounds.minChunkX - cx : cx > bounds.maxChunkX ? cx - bounds.maxChunkX : 0, cy < bounds.minChunkY ? bounds.minChunkY - cy : cy > bounds.maxChunkY ? cy - bounds.maxChunkY : 0); goblin.lod = ring <= 1 ? 'live' : ring === 2 ? 'coarse' : ring === 3 ? 'sparse' : 'sleeping'; const phase = goblin.tickPhase; const due = goblin.lod === 'live' || (goblin.lod === 'coarse' && this.simulationStep % 5 === phase % 5) || (goblin.lod === 'sparse' && this.simulationStep % 30 === phase % 30); if (due) advanceGoblin(goblin, { seed: this.seed, settlements: this.adventurerTopology.settlements, player: { x: this.px, y: this.py }, gameTime: this.gameTime }, stepSeconds); }
    for (const goblin of [...this.goblins.values()].sort((a, b) => a.id.localeCompare(b.id))) { if (goblin.state === 'fighting') continue; for (const adventurer of [...this.adventurers.values()].sort((a, b) => a.id.localeCompare(b.id))) { if (adventurer.state !== 'travelling') continue; const sampled = sampleAdventurer(adventurer, this.gameTime); const previous = sampleAdventurer(adventurer, Math.max(0, this.gameTime - stepSeconds)); if (!sampled) continue; if (!sweptContact(goblin, { x: sampled.x, y: sampled.y, previousX: previous?.x, previousY: previous?.y })) continue; const separation = Math.hypot(goblin.x - sampled.x, goblin.y - sampled.y); if (separation > 1) { const scale = 1 / separation; goblin.x = sampled.x + (goblin.x - sampled.x) * scale; goblin.y = sampled.y + (goblin.y - sampled.y) * scale; } goblin.state = 'fighting'; goblin.targetAdventurerId = adventurer.id; adventurer.state = 'fighting'; break; } }
    void stepSeconds;
  }

  private simulationStep = 0;

  private tick(dt: number) { const start = performance.now(); this.elapsed += dt; const simulationDt = dt > 0.25 ? 0 : dt; this.simAccumulator += simulationDt; let steps = 0; while (this.simAccumulator >= 0.1 && steps++ < 5) { this.simAccumulator -= 0.1; this.gameTime += 0.1; this.simulationStep++; this.simulateAdventurers(0.1); } if (steps >= 5) this.simAccumulator = 0; if (this.path.length) { const target = this.path[0]; const dx = target.x - this.px; const dy = target.y - this.py; const distance = Math.hypot(dx, dy); const amount = dt * 4.2; if (distance <= amount) { this.px = target.x; this.py = target.y; this.path.shift(); } else { this.px += (dx / distance) * amount; this.py += (dy / distance) * amount; const rotation = Math.atan2(dy, dx) * 0.08; this.hero.rotation = rotation; this.boat.rotation = rotation; } const water = this.navigation().waterRouteTileKeys.has(key(Math.round(this.px), Math.round(this.py))); this.hero.visible = !water; this.boat.visible = water; this.onStatus(this.path.length ? water ? 'Sailing' : 'Walking' : 'Exploring the Emberwild'); } const bob = this.path.length ? Math.sin(this.elapsed * 15) * 1.5 : Math.sin(this.elapsed * 2) * 0.45; this.hero.y = bob; this.boat.y = bob; this.render(); this.performance.recordFrame(performance.now() - start); }
  ...
  private renderAdventurers() {
    ...
    for (const adventurer of this.adventurers.values()) {
      const sampled = sampleAdventurer(adventurer, this.gameTime); const settlement = this.adventurerTopology?.settlements.find((candidate) => candidate.id === adventurer.currentSettlementId); const point = sampled ?? settlement; if (!point) continue;
      if (point.x < bounds.minX || point.x > bounds.maxX || point.y < bounds.minY || point.y > bounds.maxY) continue;
      visible.add(adventurer.id); let node = this.adventurerNodes.get(adventurer.id); if (!node) { node = new Graphics().circle(0, 0, 6).fill(0xd8a15c).circle(0, 0, 3).fill(0x3b2930); this.adventurerNodes.set(adventurer.id, node); this.adventurerLayer.addChild(node); }
      ...
      node.position.set((point.x + normalX * offset) * TILE_SIZE, (point.y + normalY * offset) * TILE_SIZE); node.rotation = sampled?.rotation ?? 0; node.zIndex = point.y * TILE_SIZE; node.visible = true;
    }
    for (const [id, node] of this.adventurerNodes) if (!visible.has(id)) { this.adventurerLayer.removeChild(node); node.destroy(); this.adventurerNodes.delete(id); }
    this.adventurerLayer.sortChildren();
  }
  private renderGoblins() {
    ...
    for (const goblin of this.goblins.values()) {
      ...
      visible.add(goblin.id); let node = this.goblinNodes.get(goblin.id); if (!node) { node = new Graphics().circle(0, 0, 7).fill(0x6f9b4a).circle(-3, -2, 1.4).fill(0xe7d78b).circle(3, -2, 1.4).fill(0xe7d78b).moveTo(-5, 4).lineTo(0, 7).lineTo(5, 4).stroke({ color: 0x273c2c, width: 1.5 }); this.goblinNodes.set(goblin.id, node); this.goblinLayer.addChild(node); }
      node.position.set(goblin.x * TILE_SIZE, goblin.y * TILE_SIZE); node.rotation = Math.atan2(goblin.y - goblin.previousY, goblin.x - goblin.previousX); node.zIndex = goblin.y * TILE_SIZE; node.visible = true;
    }
    for (const [id, node] of this.goblinNodes) if (!visible.has(id)) { this.goblinLayer.removeChild(node); node.destroy(); this.goblinNodes.delete(id); }
    this.goblinLayer.sortChildren();
  }
  private render() { const centerX = this.app.screen.width / 2; const centerY = this.app.screen.height / 2; const cameraX = centerX - this.px * TILE_SIZE; const cameraY = centerY - this.py * TILE_SIZE; this.world.position.set(cameraX, cameraY); this.mapOverlay.position.set(cameraX, cameraY); this.actor.position.set(centerX + TILE_SIZE / 2, centerY + TILE_SIZE * 2 / 3 - HERO_FEET_OFFSET); const bounds = this.visibleBounds(); const signature = `${bounds.minChunkX},${bounds.maxChunkX},${bounds.minChunkY},${bounds.maxChunkY}`; if (this.streamDirty || signature !== this.lastStreamBounds) { this.streamDirty = false; this.lastStreamBounds = signature; this.syncStreaming(bounds); } this.renderAdventurers(); this.renderGoblins(); this.publishTileDebug(); this.performance.setDisplayObjects(this.world.children.reduce((count, child) => count + child.children.length + 1, 0)); }
