...
import { drawBuilding, drawEdgeFeature, drawLandmark, drawRoad, drawTree } from './featureRenderer';
...
export class Game {
  ...
  private updateDetail(display: ChunkDisplay, detail: Detail) {
    ...
    for (const layout of chunk.settlementLayouts) { for (const building of layout.buildings) drawBuilding(objectLayer, building); for (const edge of layout.edgeFeatures) drawEdgeFeature(objectLayer, edge); }
