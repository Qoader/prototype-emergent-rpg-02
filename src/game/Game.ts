...
import { CHUNK_SIZE, createWorldConfig, findPath, findStartingPosition, key, TILE_SIZE, tileAt, worldToChunk, worldToRegion, type TerrainChunk, type Tile, type WorldChunk } from './world';
...
export class Game {
  ...
  private waitForPaint() { return new Promise<void>((resolve) => requestAnimationFrame(() => resolve())); }
  private async bootstrap() {
    const playerChunk = { cx: Math.floor(this.px / CHUNK_SIZE), cy: Math.floor(this.py / CHUNK_SIZE) };
    try {
      const terrain = await this.provider.getTerrainChunk(playerChunk.cx, playerChunk.cy); if (this.destroyed) return;
      this.drawChunk(terrain, 'terrain'); performance.mark('emberwild-player-terrain-ready'); performance.measure('emberwild-player-terrain', 'emberwild-canvas-ready', 'emberwild-player-terrain-ready');
      await this.waitForPaint(); if (this.destroyed) return;
      const full = await this.provider.getChunk(playerChunk.cx, playerChunk.cy); if (this.destroyed) return;
      this.drawChunk(full, this.detailFor(full)); this.playerChunkNavigationReady = true; performance.mark('emberwild-player-navigation-ready'); performance.measure('emberwild-player-navigation', 'emberwild-player-terrain-ready', 'emberwild-player-navigation-ready'); this.onStatus('Exploring the Emberwild');
      const bounds = this.visibleBounds(); const visibleTerrain = [...streamingPlan(bounds, playerChunk).requests.filter((request) => !request.preload && (request.cx !== playerChunk.cx || request.cy !== playerChunk.cy))];
      await Promise.all(visibleTerrain.map(async (request) => { const chunk = await this.provider.getTerrainChunk(request.cx, request.cy); if (!this.destroyed) this.drawChunk(chunk, 'terrain'); }));
      if (this.destroyed) return; performance.mark('emberwild-visible-terrain-ready'); performance.measure('emberwild-visible-terrain', 'emberwild-canvas-ready', 'emberwild-visible-terrain-ready'); this.bootstrapping = false; this.streamDirty = true; this.render();
  ...
  private detailFor(chunk: RenderableChunk): Detail { const playerChunkX = Math.floor(this.px / CHUNK_SIZE); const playerChunkY = Math.floor(this.py / CHUNK_SIZE); const distance = Math.max(Math.abs(chunk.cx - playerChunkX), Math.abs(chunk.cy - playerChunkY)); return distance <= 1 ? 'full' : distance <= 2 ? 'medium' : 'terrain'; }
  ...
  private markVisibleFullReady() { if (this.bootstrapping || this.visibleFullMarked) return; const bounds = this.visibleBounds(); const plan = streamingPlan(bounds, { cx: Math.floor(this.px / CHUNK_SIZE), cy: Math.floor(this.py / CHUNK_SIZE) }); if (![...plan.visible].every((chunkKey) => this.displays.get(chunkKey)?.chunk.detail === 'full')) return; this.visibleFullMarked = true; performance.mark('emberwild-visible-full-ready'); performance.measure('emberwild-visible-full', 'emberwild-canvas-ready', 'emberwild-visible-full-ready'); }
