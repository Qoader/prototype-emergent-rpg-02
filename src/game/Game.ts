...
import { CHUNK_SIZE, chunkBounds, chunkGridOrigin, createWorldConfig, findPath, findStartingPosition, key, TILE_SIZE, tileAt, worldToChunk, worldToRegion, type ChunkGridOrigin, type TerrainChunk, type Tile, type WorldChunk, type WorldConfig } from './world';
...
interface ChunkDisplay { chunk: WorldChunk; terrain: Sprite; texture: RenderTexture; roadLayer: Container; citySurfaceLayer: Container; objectLayer: Container; objectNodes: RenderNode[]; }
...
export class Game {
  private app = new Application(); private world = new Container(); private terrainLayer = new Container(); private roadLayer = new Container(); private citySurfaceLayer = new Container(); private objectLayer = new Container(); private adventurerLayer = new Container(); private goblinLayer = new Container(); private mapOverlay = new Container(); private actor = new Container(); private hero!: ActorSprite; private boat = new Container(); private marker = new Graphics(); private seed: string; private config: WorldConfig; private gridOrigin: ChunkGridOrigin; private onStatus: Status; private onTileDebug: TileDebug; private px = 0; private py = 0; private path: Tile[] = []; private displays = new Map<string, ChunkDisplay>(); private prepared = new Map<string, ChunkDisplay>(); private streamDirty = true; private lastStreamBounds = ''; private provider: WorldWorkerClient; private performance = new PerformanceMonitor(); private destroyed = false; private bootstrapping = true; private playerChunkNavigationReady = false; private visibleFullMarked = false; private elapsed = 0; private gameTime = 0; private simAccumulator = 0; private lastTileDebugSignature = ''; private lastDebugAt = 0; private lastMetricsAt = 0; private scheduler: ChunkStreamingScheduler; private readonly chunkLayerKeys = new WeakMap<Container, string>(); private readonly navCounts = { road: new Map<string, number>(), bridge: new Map<string, number>(), waterRoute: new Map<string, number>(), port: new Map<string, number>(), blocked: new Map<string, number>() }; private readonly navigationIndex = { roadTileKeys: new Set<string>(), bridgeTileKeys: new Set<string>(), waterRouteTileKeys: new Set<string>(), portTileKeys: new Set<string>(), portLinks: new Map<string, Set<string>>(), blockedTileKeys: new Set<string>() }; private readonly navigationContributions = new Map<string, NavigationContribution>(); private adventurerTopology: TravelTopology | null = null; private adventurerTopologyKey = ''; private adventurerTopologyPromise: Promise<void> | null = null; private adventurers = new Map<string, Adventurer>(); private adventurerNodes = new Map<string, ActorSprite>(); private goblins = new Map<string, Goblin>(); private goblinNodes = new Map<string, ActorSprite>(); private adventurerLayerDirty = false; private goblinLayerDirty = false;
  constructor(private host: HTMLElement, seed: string, onStatus: Status, onTileDebug: TileDebug = () => undefined) { this.seed = seed; this.config = createWorldConfig(seed); this.gridOrigin = chunkGridOrigin(this.config); const startingPosition = findStartingPosition(this.config); this.px = startingPosition.x; this.py = startingPosition.y; this.onStatus = onStatus; this.onTileDebug = onTileDebug; this.objectLayer.sortableChildren = true; this.provider = new WorldWorkerClient(this.config); this.scheduler = new ChunkStreamingScheduler({ isLoaded: (chunkKey) => this.displays.has(chunkKey) || this.prepared.has(chunkKey), load: (request) => this.loadChunk(request), receive: (chunk, preload) => this.receiveChunk(chunk, preload), failed: () => this.onStatus('World generation delayed') }); void this.init(); }
  private async init() { try { await this.app.init({ resizeTo: this.host, background: '#294942', antialias: true, preference: 'webgl', resolution: Math.min(devicePixelRatio, 2) }); await this.provider.whenReady(); } catch { this.onStatus('WebGL or the world worker is required to enter the Emberwild'); return; } this.host.appendChild(this.app.canvas); performance.mark('emberwild-canvas-ready'); performance.mark('emberwild-worker-ready'); this.world.addChild(this.terrainLayer, this.roadLayer, this.citySurfaceLayer, this.objectLayer, this.adventurerLayer, this.goblinLayer); this.mapOverlay.addChild(this.marker); this.app.stage.addChild(this.world, this.mapOverlay, this.actor); this.buildHero(); this.app.stage.eventMode = 'static'; this.app.stage.hitArea = this.app.screen; this.app.stage.on('pointertap', (event) => this.go(event.global.x, event.global.y)); this.app.ticker.add((ticker) => this.tick(ticker.deltaMS / 1000)); window.addEventListener('resize', this.resize); this.onStatus('Preparing the Emberwild…'); this.resize(); void this.bootstrap(); }
  private async bootstrap() {
    const playerChunk = worldToChunk(Math.round(this.px), Math.round(this.py), this.gridOrigin);
    try {
      const full = await this.provider.getChunk(playerChunk.cx, playerChunk.cy); if (this.destroyed) return;
      this.commitChunk(this.prepareChunk(full)); this.playerChunkNavigationReady = true; performance.mark('emberwild-player-chunk-ready'); performance.measure('emberwild-player-chunk', 'emberwild-canvas-ready', 'emberwild-player-chunk-ready'); this.onStatus('Exploring the Emberwild');
      this.bootstrapping = false; this.streamDirty = true; this.render();
    } catch { if (!this.destroyed) this.onStatus('World generation delayed'); }
  }
  private buildHero() {
  ...
  async transportTo(destination: NearbySettlement) {
    const chunkCoordinate = worldToChunk(destination.gateX, destination.gateY, this.gridOrigin); this.path = []; this.marker.clear(); this.onStatus(`Travelling to ${destination.name}`);
    try {
      const chunk = await this.provider.getChunk(chunkCoordinate.cx, chunkCoordinate.cy); if (this.destroyed) return;
      const prepared = this.prepareChunk(chunk); this.px = destination.gateX; this.py = destination.gateY; this.commitChunk(prepared); this.lastTileDebugSignature = ''; this.streamDirty = true; this.render(); this.onStatus(`Arrived at ${destination.name}`);
  ...
  private simulateAdventurers(stepSeconds: number) {
    if (this.bootstrapping) return; this.refreshAdventurerTopology(); if (!this.adventurerTopology) return; const bounds = this.visibleBounds();
  ...
  private render() { const centerX = this.app.screen.width / 2; const centerY = this.app.screen.height / 2; const cameraX = centerX - this.px * TILE_SIZE; const cameraY = centerY - this.py * TILE_SIZE; this.world.position.set(cameraX, cameraY); this.mapOverlay.position.set(cameraX, cameraY); this.actor.position.set(centerX + TILE_SIZE / 2, centerY + TILE_SIZE * 2 / 3 - HERO_FEET_OFFSET); const bounds = this.visibleBounds(); const signature = `${bounds.minChunkX},${bounds.maxChunkX},${bounds.minChunkY},${bounds.maxChunkY}`; if (!this.bootstrapping && (this.streamDirty || signature !== this.lastStreamBounds)) { this.streamDirty = false; this.lastStreamBounds = signature; this.syncStreaming(bounds); } this.renderAdventurers(); this.renderGoblins(); const now = performance.now(); if (now - this.lastDebugAt >= 250) { this.lastDebugAt = now; this.publishTileDebug(); } if (now - this.lastMetricsAt >= 1000) { this.lastMetricsAt = now; this.performance.setDisplayObjects(this.world.children.reduce((count, child) => count + child.children.length + 1, 0)); } }
  private publishTileDebug() { const x = Math.round(this.px); const y = Math.round(this.py); const coordinate = worldToChunk(x, y, this.gridOrigin); const displayChunk = this.displays.get(key(coordinate.cx, coordinate.cy))?.chunk; const chunk = displayChunk?.detail === 'full' ? displayChunk : undefined; const signature = `${x},${y}:${chunk ? `${chunk.cx},${chunk.cy}` : 'pending'}`; if (signature === this.lastTileDebugSignature) return; this.lastTileDebugSignature = signature; this.onTileDebug(tileDebugInfo(tileAt(this.seed, x, y), chunk)); }
  private visibleBounds() { return visibleWorldBounds({ x: this.px, y: this.py }, this.app.screen, this.gridOrigin); }
  private syncStreaming(bounds: ChunkBounds) { const plan = streamingPlan(bounds, worldToChunk(Math.floor(this.px), Math.floor(this.py), this.gridOrigin));
    for (const [chunkKey, display] of this.displays) if (!plan.visible.has(chunkKey)) this.evict(chunkKey, display);
    for (const [chunkKey, display] of this.prepared) if (!plan.visible.has(chunkKey) && !plan.preload.has(chunkKey)) { this.prepared.delete(chunkKey); this.destroyPreparedChunk(display); }
    for (const chunkKey of plan.visible) { const display = this.prepared.get(chunkKey); if (display) { this.prepared.delete(chunkKey); this.commitChunk(display); } }
    this.scheduler.setPlan(plan);
  }
  ...
  private receiveChunk(chunk: WorldChunk, preload: boolean) { const chunkKey = key(chunk.cx, chunk.cy); const prepared = this.prepareChunk(chunk); if (preload) this.prepared.set(chunkKey, prepared); else { this.commitChunk(prepared); this.markVisibleFullReady(); } }
  private markVisibleFullReady() { if (this.bootstrapping || this.visibleFullMarked) return; const bounds = this.visibleBounds(); const plan = streamingPlan(bounds, worldToChunk(Math.floor(this.px), Math.floor(this.py), this.gridOrigin)); if (![...plan.visible].every((chunkKey) => this.displays.get(chunkKey)?.chunk.detail === 'full')) return; this.visibleFullMarked = true; performance.mark('emberwild-visible-full-ready'); performance.measure('emberwild-visible-full', 'emberwild-canvas-ready', 'emberwild-visible-full-ready'); }
  ...
  private drawTerrain(chunk: Pick<TerrainChunk, 'cx' | 'cy' | 'tiles'>) {
    ...
    const bounds = chunkBounds(chunk.cx, chunk.cy, this.gridOrigin);
    for (const tile of chunk.tiles) {
      const localX = (tile.x - bounds.minX) * TILE_SIZE; const localY = (tile.y - bounds.minY) * TILE_SIZE; const tone = BIOME_COLORS[tile.biome]; const variation = this.variation('terrain', tile.x, tile.y);
  ...
  /** Builds every static resource without attaching it to the live scene. */
  private prepareChunk(chunk: WorldChunk): ChunkDisplay {
    const texture = this.drawTerrain(chunk); const terrain = new Sprite(texture); const bounds = chunkBounds(chunk.cx, chunk.cy, this.gridOrigin); terrain.position.set(bounds.minX * TILE_SIZE, bounds.minY * TILE_SIZE);
    const display: ChunkDisplay = { chunk, terrain, texture, roadLayer: new Container(), citySurfaceLayer: new Container(), objectLayer: new Container(), objectNodes: [] };
    try { this.buildFullDetail(display); return display; }
    catch (error) { this.destroyPreparedChunk(display); throw error; }
  }
  /** Publishes a completely prepared chunk in one synchronous scene update. */
  private commitChunk(display: ChunkDisplay) {
    const chunkKey = key(display.chunk.cx, display.chunk.cy); const existing = this.displays.get(chunkKey); if (existing) this.evict(chunkKey, existing);
    this.insertChunkLayer(this.terrainLayer, display.terrain, chunkKey); this.insertChunkLayer(this.roadLayer, display.roadLayer, chunkKey); this.insertChunkLayer(this.citySurfaceLayer, display.citySurfaceLayer, chunkKey);
    for (const node of display.objectLayer.removeChildren()) this.objectLayer.addChild(node);
    this.objectLayer.sortChildren(); this.displays.set(chunkKey, display); this.indexChunk(display.chunk, true); this.lastTileDebugSignature = '';
  }
  private destroyPreparedChunk(display: ChunkDisplay) { display.roadLayer.destroy({ children: true }); display.citySurfaceLayer.destroy({ children: true }); display.objectLayer.destroy({ children: true }); display.terrain.destroy(); display.texture.destroy(true); }
  ...
  private captureObjects(display: ChunkDisplay, draw: () => void) { const before = display.objectLayer.children.length; draw(); display.objectNodes.push(...display.objectLayer.children.slice(before)); }
  private clearObjects(display: ChunkDisplay) { for (const node of display.objectNodes) { node.parent?.removeChild(node); node.destroy(); } display.objectNodes = []; }
  private buildFullDetail(display: ChunkDisplay) {
    const { chunk } = display; const { roadLayer, citySurfaceLayer } = display; const bounds = chunkBounds(chunk.cx, chunk.cy, this.gridOrigin);
    ...
    for (const road of chunk.roads) for (const bridge of road.bridges) candidates.push({ id: bridge.id, tiles: bridge.tiles, points: bridge.points, width: bridge.width, color: 0xc98b53, priority: 1000 + bridge.width });
    for (const layout of chunk.settlementLayouts) for (const street of layout.streets) {
    ...
    for (const [portKey, waterTiles] of drawnPorts) { const [x, y] = portKey.split(',').map(Number); this.captureObjects(display, () => drawPort(display.objectLayer, x, y, waterTiles)); }
    for (const layout of chunk.settlementLayouts) for (const square of layout.plazas) drawPlaza(citySurfaceLayer, square, bounds);
    ...
    for (const tile of chunk.tiles) { const density = this.variation('props', tile.x, tile.y); if (tile.road || intramural.has(key(tile.x, tile.y))) continue; if (tile.landmark && tile.walkable) this.captureObjects(display, () => drawLandmark(display.objectLayer, tile)); else if (tile.walkable && tile.biome === 'forest' && density > 0.89) this.captureObjects(display, () => drawTree(display.objectLayer, tile.x, tile.y, density)); else if (tile.walkable && (tile.biome === 'grassland' || tile.biome === 'swamp') && density > 0.9775) this.captureObjects(display, () => drawTree(display.objectLayer, tile.x, tile.y, 0.7)); }
    for (const layout of chunk.settlementLayouts) { if (layout.fortification) this.captureObjects(display, () => drawFortification(display.objectLayer, layout.fortification!, bounds)); for (const building of layout.buildings) if (building.x >= bounds.minX && building.x <= bounds.maxX && building.y >= bounds.minY && building.y <= bounds.maxY) this.captureObjects(display, () => drawBuilding(display.objectLayer, building)); for (const edge of layout.edgeFeatures) if (edge.x <= bounds.maxX && edge.x + edge.width - 1 >= bounds.minX && edge.y <= bounds.maxY && edge.y + edge.height - 1 >= bounds.minY) this.captureObjects(display, () => drawEdgeFeature(display.objectLayer, edge)); }
  }
  ...
  destroy() { this.destroyed = true; this.scheduler.stop(); window.removeEventListener('resize', this.resize); for (const [chunkKey, display] of this.displays) this.evict(chunkKey, display); for (const display of this.prepared.values()) this.destroyPreparedChunk(display); this.prepared.clear(); void this.provider.dispose(); this.app.destroy(true, { children: true }); }
}
