...
type RenderableChunk = TerrainChunk | WorldChunk;
interface ChunkDisplay { chunk: RenderableChunk; terrain: Sprite; texture: RenderTexture; roadLayer: Container; citySurfaceLayer: Container; objectNodes: RenderNode[]; detail: Detail | null; }
...
export class Game {
  ...
  private async bootstrap() {
    ...
    try {
      ...
      this.drawChunk(full, this.detailFor(full)); this.playerChunkNavigationReady = true; performance.mark('emberwild-player-chunk-ready'); performance.measure('emberwild-player-chunk', 'emberwild-canvas-ready', 'emberwild-player-chunk-ready'); this.onStatus('Exploring the Emberwild');
  ...
  private detailFor(chunk: RenderableChunk): Detail { const playerChunk = worldToChunk(Math.floor(this.px), Math.floor(this.py), this.gridOrigin); const distance = Math.max(Math.abs(chunk.cx - playerChunk.cx), Math.abs(chunk.cy - playerChunk.cy)); return distance <= 1 ? 'full' : distance <= 2 ? 'medium' : 'terrain'; }
  ...
  private receiveChunk(chunk: WorldChunk, preload: boolean) { this.indexChunk(chunk, true); if (preload) this.prefetched.set(key(chunk.cx, chunk.cy), chunk); else { this.drawChunk(chunk, this.detailFor(chunk)); this.markVisibleFullReady(); } }
  ...
  private drawChunk(chunk: RenderableChunk, detail: Detail) { const chunkKey = key(chunk.cx, chunk.cy); const existing = this.displays.get(chunkKey); if (existing) { if (chunk.detail === 'full') this.indexChunk(chunk, true); existing.chunk = chunk; this.updateDetail(existing, chunk.detail === 'full' ? detail : 'terrain'); this.lastTileDebugSignature = ''; return; } if (chunk.detail === 'full') this.indexChunk(chunk, true); const texture = this.drawTerrain(chunk); const terrain = new Sprite(texture); const bounds = chunkBounds(chunk.cx, chunk.cy, this.gridOrigin); terrain.position.set(bounds.minX * TILE_SIZE, bounds.minY * TILE_SIZE); const roadLayer = new Container(); const citySurfaceLayer = new Container(); const display: ChunkDisplay = { chunk, terrain, texture, roadLayer, citySurfaceLayer, objectNodes: [], detail: null }; this.insertChunkLayer(this.terrainLayer, terrain, chunkKey); this.insertChunkLayer(this.roadLayer, roadLayer, chunkKey); this.insertChunkLayer(this.citySurfaceLayer, citySurfaceLayer, chunkKey); this.displays.set(chunkKey, display); this.updateDetail(display, chunk.detail === 'full' ? detail : 'terrain'); this.lastTileDebugSignature = ''; }
  ...
  private updateDetail(display: ChunkDisplay, detail: Detail) {
    const { chunk } = display; const resolvedDetail = chunk.detail === 'full' ? detail : 'terrain'; if (display.detail === resolvedDetail) return; display.detail = resolvedDetail; display.roadLayer.removeChildren().forEach((child) => child.destroy()); display.citySurfaceLayer.removeChildren().forEach((child) => child.destroy()); this.clearObjects(display); if (resolvedDetail === 'terrain') return; const { roadLayer, citySurfaceLayer } = display; const bounds = chunkBounds(chunk.cx, chunk.cy, this.gridOrigin);
    if (chunk.detail !== 'full') return;
