...
export class Game {
  ...
  private refreshAdventurerTopology() {
    ...
    this.adventurerTopologyKey = requestKey; this.adventurerTopologyPromise = this.provider.getTravelTopology(cell.gx, cell.gy, 1).then((topology) => { if (this.destroyed) return; this.adventurerTopology = topology; for (const settlement of topology.settlements) { const adventurer = createAdventurer(settlement, 0); if (adventurer && !this.adventurers.has(adventurer.id)) { if (this.gameTime > 0) advanceAdventurer(adventurer, topology, this.gameTime); this.adventurers.set(adventurer.id, adventurer); } } }).catch(() => undefined).finally(() => { this.adventurerTopologyPromise = null; });
  }
