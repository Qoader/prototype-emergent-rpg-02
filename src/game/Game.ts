...
export class Game {
  ...
  private addRoad(features: Container, points: Array<{ x: number; y: number }>, width: number, color: number) { if (!points.length) return; const road = new Graphics().moveTo(points[0].x * TILE_SIZE, points[0].y * TILE_SIZE); for (const point of points.slice(1)) road.lineTo(point.x * TILE_SIZE, point.y * TILE_SIZE); if (points.length === 1) road.circle(points[0].x * TILE_SIZE, points[0].y * TILE_SIZE, Math.max(2, width * TILE_SIZE / 8)); road.stroke({ color: 0x3b342b, width: width * TILE_SIZE / 4 + 3, alpha: 0.25 }); road.stroke({ color, width: width * TILE_SIZE / 4, alpha: 1 }); road.zIndex = points[points.length - 1].y * TILE_SIZE - 2; features.addChild(road); }
  private addRoadJunction(features: Container, junction: { x: number; y: number; width: number; color: number }) {
    const x = junction.x * TILE_SIZE; const y = junction.y * TILE_SIZE;
    const surfaceRadius = junction.width * TILE_SIZE / 8; const outlineRadius = (junction.width * TILE_SIZE / 4 + 3) / 2;
    const cap = new Graphics().circle(x, y, outlineRadius).fill({ color: 0x3b342b, alpha: 0.25 }).circle(x, y, surfaceRadius).fill({ color: junction.color, alpha: 1 });
    features.addChild(cap);
  }
  ...
  private updateDetail(display: ChunkDisplay, detail: Detail) {
    ...
    for (const road of composition.roads) this.addRoad(roadLayer, road.points, road.width, road.color);
    for (const junction of composition.junctions) this.addRoadJunction(roadLayer, junction);
