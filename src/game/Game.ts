...
export class Game {
  ...
  private renderAdventurers() {
    ...
    for (const adventurer of this.adventurers.values()) {
      ...
      visible.add(adventurer.id); let node = this.adventurerNodes.get(adventurer.id); if (!node) { node = createActorSprite('adventurer', variantForId(adventurer.id)); this.adventurerNodes.set(adventurer.id, node); this.adventurerLayer.addChild(node); }
      ...
      const x = point.x + normalX * offset + 0.5; const y = point.y + normalY * offset + 0.5; node.position.set(x * TILE_SIZE, y * TILE_SIZE); const moving = Boolean(sampled && adventurer.state === 'travelling'); const heading = sampled?.rotation ?? 0; applyActorMotion(node, actorMotion(moving, heading, this.elapsed, variantForId(adventurer.id).phase)); node.zIndex = y * TILE_SIZE; node.visible = true;
    }
    for (const [id, node] of this.adventurerNodes) if (!visible.has(id)) { this.adventurerLayer.removeChild(node); node.destroy({ children: true }); this.adventurerNodes.delete(id); }
  ...
  private renderGoblins() {
    ...
    for (const goblin of this.goblins.values()) {
      ...
      visible.add(goblin.id); let node = this.goblinNodes.get(goblin.id); if (!node) { node = createActorSprite('goblin', variantForId(goblin.id)); this.goblinNodes.set(goblin.id, node); this.goblinLayer.addChild(node); }
      const dx = goblin.x - goblin.previousX; const dy = goblin.y - goblin.previousY; const moving = Math.hypot(dx, dy) > 1e-4 && goblin.state !== 'fighting'; const heading = moving ? Math.atan2(dy, dx) : 0; const x = goblin.x + 0.5; const y = goblin.y + 0.5; node.position.set(x * TILE_SIZE, y * TILE_SIZE); applyActorMotion(node, actorMotion(moving, heading, this.elapsed, variantForId(goblin.id).phase)); node.zIndex = y * TILE_SIZE; node.visible = true;
    }
    for (const [id, node] of this.goblinNodes) if (!visible.has(id)) { this.goblinLayer.removeChild(node); node.destroy({ children: true }); this.goblinNodes.delete(id); }
