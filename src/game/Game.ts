...
export class Game {
  ...
  private renderAdventurers() {
    ...
    for (const adventurer of this.adventurers.values()) {
      ...
      const x = point.x + normalX * offset + 0.5; const y = point.y + normalY * offset + 0.5; node.position.set(x * TILE_SIZE, y * TILE_SIZE); const moving = Boolean(sampled && adventurer.state === 'travelling'); const heading = sampled?.rotation ?? 0; applyActorMotion(node, actorMotion(moving, heading, this.elapsed, variantForId(adventurer.id).phase)); const zIndex = y * TILE_SIZE; if (node.zIndex !== zIndex) this.adventurerLayerDirty = true; node.zIndex = zIndex; node.visible = true;
    }
