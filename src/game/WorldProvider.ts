...
export class WorldProvider {
  ...
  getChunk(cx: number, cy: number): Promise<WorldChunk> {
    ...
    const request = Promise.resolve().then(async () => {
      ...
      const blockedTiles = new Set<string>(); const gateTiles = new Set<string>(); const intramuralTiles = new Set<string>(); const buildingTiles = new Set<string>();
      ...
      const composedTiles = chunk.tiles.map((tile) => { const tileKey = `${tile.x},${tile.y}`; const port = portTiles.has(tileKey); const waterRoute = waterRouteTiles.has(tileKey); const road = roadTiles.has(tileKey) || gateTiles.has(tileKey); const blocked = !port && (blockedTiles.has(tileKey) || (buildingTiles.has(tileKey) && !road)); if (!road && !blocked && !intramuralTiles.has(tileKey) && !port && !waterRoute) return tile; return { ...tile, road, port, waterRoute, walkable: port ? true : blocked ? false : tile.walkable, movementCost: port ? 1 : blocked ? Infinity : tile.movementCost, landmark: road || port || waterRoute || intramuralTiles.has(tileKey) ? (tile.landmark === 'tree' ? null : tile.landmark) : tile.landmark }; });
