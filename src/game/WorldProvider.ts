...
export class WorldProvider {
  ...
  getChunk(cx: number, cy: number): Promise<WorldChunk> {
    ...
    const request = Promise.resolve().then(async () => {
      ...
      const composedTiles = chunk.tiles.map((tile) => { const tileKey = `${tile.x},${tile.y}`; const road = roadTiles.has(tileKey) || gateTiles.has(tileKey); const blocked = blockedTiles.has(tileKey) || (buildingTiles.has(tileKey) && !road); if (!road && !blocked && !intramuralTiles.has(tileKey)) return tile; return { ...tile, road, walkable: blocked ? false : tile.walkable, movementCost: blocked ? Infinity : tile.movementCost, landmark: road || intramuralTiles.has(tileKey) ? (tile.landmark === 'tree' ? null : tile.landmark) : tile.landmark }; });
